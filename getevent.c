    ...
    for(i = EV_KEY; i <= EV_MAX; i++) { // skip EV_SYN since we cannot query its available codes
        ...
        while(1) {
            ...
            if(res < bits_size)
                break;
            bits_size = res + 16;
            uint8_t *new_bits = realloc(bits, bits_size * 2);
            if(new_bits == NULL) {
            ...
            for(k = 0; k < 8; k++)
                if(bits[j] & 1 << k) {
                    ...
                    if(bit_labels && (print_flags & PRINT_LABELS)) {
                        ...
                        if(bit_label) {
                            int label_len = (int)strlen(bit_label);
                            int pad = label_len < 20 ? 20 - label_len : 0;
                            printf(" %.20s%c%*s", bit_label, down, pad, "");
                        }
                        else
...
static void print_event(int type, int code, int value, int print_flags)
{
    ...
    if (print_flags & PRINT_LABELS) {
        ...
        printf("%04x %04x %08x", type, code, (unsigned int)value);
    }
}
...
static int close_device(const char *device, int print_flags)  /* fix: added missing 'static' */
{
    int i;
    for(i = 1; i < nfds; i++) {
    ...
    if(print_flags & PRINT_DEVICE_ERRORS)
        fprintf(stderr, "remove device: %s not found\n", device);
    return -1;
}
...
int main(int argc, char *argv[])
{
    ...
    if(device) {
        ...
        print_device = 1;
        res = inotify_add_watch(ufds[0].fd, device_path, IN_DELETE | IN_CREATE);  /* fix: was indented with tabs */
        if(res < 0) {
    ...
    if(get_switch) {
        for(i = 1; i < nfds; i++) {
            uint16_t sw;
            res = ioctl(ufds[i].fd, EVIOCGSW(sizeof(sw)), &sw);
            if(res < 0) {
        ...
        for(i = 1; i < nfds; i++) {
            if(ufds[i].revents) {
                if(ufds[i].revents & POLLIN) {
                    ...
                    if(get_time) {
                        printf("[%8ld.%06ld] ", (long)event.input_event_sec, (long)event.input_event_usec);
                    }
                    if(print_device)
                    ...
                    if(sync_rate && event.type == 0 && event.code == 0) {
                        ...
                        if(last_sync_time && now != last_sync_time)
                            printf(" rate %" PRId64, 1000000LL / (now - last_sync_time));
                        last_sync_time = now;
                    }
                    printf("%s", newline);
